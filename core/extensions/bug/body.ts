...
// Scale the whole creature to 85% of its previous size.
export const CREATURE_SIZE_SCALE = 0.85;
...
const LEG_LINK_LENGTHS: readonly [number, number, number] = [
  28 * CREATURE_SIZE_SCALE,
  34 * CREATURE_SIZE_SCALE,
  42 * CREATURE_SIZE_SCALE,
];
...
export class CaterpillarBody {
    ...
    for (const leg of this.legs) {
      ...
      if (leg.mode === "stance") {
        ...
        if (leg.mode === "swing") {
          ...
          if (
            handError < 6 * CREATURE_SIZE_SCALE ||
            leg.modeAge > reachDeadline
          ) {
            leg.mode = "search";
            leg.modeAge = 0;
            leg.elevationTarget = 0;
    ...
    for (const leg of this.legs) {
      ...
      if (leg.mode === "search") {
        ...
        if (
          leg.modeAge > 0.025 &&
          leg.elevation < 0.28 &&
          handError < 4.5 * CREATURE_SIZE_SCALE
        ) {
          this.plantLeg(leg);
  ...
  private wallAwareDirection(
    ...
    const padding = this.baseRadius + 7 * CREATURE_SIZE_SCALE;
    const margin = 72;
    const edge = padding + 5 * CREATURE_SIZE_SCALE;
  ...
  private advanceAlternatingGait(
    ...
    const completed = activeLegs.every((leg) => {
      ...
      return (
        leg.contraction > 0.76 &&
        distanceToHand <= pullTarget + 3.5 * CREATURE_SIZE_SCALE
      );
    });
  ...
  private clampHandTarget(target: Vec2, bounds: Rect): Vec2 {
    const padding = 8 * CREATURE_SIZE_SCALE;
    return {
  ...
  private renderChewOffsetAt(index: number, interpolation: number): Vec2 {
    ...
    const rearward = index / Math.max(1, this.nodes.length - 1);
    const amplitude =
      lerp(8.5, 4.8, rearward) * CREATURE_SIZE_SCALE * excitement;
    const sideways =
    ...
    for (let firstIndex = 0; firstIndex < this.nodes.length; firstIndex += 1) {
      for (
        ...
        const minimumDistance =
          firstRadius + secondRadius + 4 * CREATURE_SIZE_SCALE;
